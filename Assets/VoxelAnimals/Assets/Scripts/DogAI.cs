...
public class DogAI : MonoBehaviour
{
    ...
    public float walkingSpeed = 5f; //how Fast should it walk
    public float runningSpeed = 33f;//how Fast should it run
        ...
        else if (currentState == AIState.Jumping)
        {
            if (switchAction)
            {
                ...
                if(!animator || animator.GetCurrentAnimatorStateInfo(0).normalizedTime - Mathf.Floor(animator.GetCurrentAnimatorStateInfo(0).normalizedTime) > 0.99f)
                {
                    ...
                    agent.destination = RandomNavSphere(transform.position, Random.Range(5, 27));
